      ...
      {state.data.deleted && (
        <tr>
          ...
          <td className="px-4 py-2 text-gray-700 whitespace-nowrap flex flex-row text-[25px] gap-x-[20px]">
            <FaRegEdit onClick={() => handleEdit(state.data.slug)} className="cursor-pointer" />
            <FaTrashRestoreAlt onClick={() => handleTrash(state.data.id)} className="cursor-pointer" />
            <button onClick={() => props.deleteBrand(state.data.id)} type='button' className="bg-danger text-white p-[2px] rounded-md">
              <FaTimes />
            </button>
          </td>
        </tr>
