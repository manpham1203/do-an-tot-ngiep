...
namespace BLL.Product
{
    public class ProductFullBLL
    {
        ...
        public async Task<ProductFullVM> GetById(string id)
        {
            ...
            var listImg = await productImageBLL.GetByObjectId(productFullVM.Id, objectType);
            if (listImg.Count > 0)
            {
                productFullVM.PictureVMs = new List<PictureVM>();
                for (int i = 0; i < listImg.Count; i++)
                {
                    productFullVM.PictureVMs.Add(listImg[i]);
                }
            }
        ...
        public async Task<ProductFullVM> GetBySlug(string slug)
        {
            ...
            var productImageBLL = new PictureBLL();
            var listImg = await productImageBLL.GetByObjectId(productFullVM.Id, "product");
            if (listImg.Count > 0)
            {
                productFullVM.PictureVMs = new List<PictureVM>();
                productFullVM.PictureVMs.AddRange(listImg);
            }
        ...
        //        var productImageBLL = new PictureBLL();
        //        var listImg = await productImageBLL.GetByObjectId(productFullVM[i].Id, objectType);
        //        if (listImg.Count > 0)
        //        {
